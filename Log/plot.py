...
for i in range(1,4):
    for j in range(8):
        axs[int(j%4), int(j/4)].plot(time, a_pre[:,i+j*3],'.-', label=lab_pre[i])
        axs[int(j%4), int(j/4)].plot(time, a_out[:,i+j*3],'.-', label=lab_out[i])
for j in range(8):
    # axs[j].set_xlim(386,389)
    axs[int(j%4), int(j/4)].grid()
    axs[int(j%4), int(j/4)].legend()
plt.grid()
#######for ikfom#######
